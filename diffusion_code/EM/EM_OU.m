function theta_hat = EM_OU(bdryPts_t,mu,dt_small,dt_large,sampleSize,nofIt)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
for it = 1:nofIt 
  ...
  for k = 1:sampleSize  
	...
	% exact sampling
    % temp = OU_bridge(bdryPts_t(:,t),bdryPts_t(:,t+1),mu,theta_hat(it,1),dt_small,dt_large,1);
    %size(temp)
	

       
	logL(k,1) = logL(k,1) + logLikelihood(x_dense(:,(t-1)*N+1:t*N),theta_hat(it,1),mu,dt_small,type);
    end
%       plot(x_dense)
%    	size(x_dense)
%    	pause()
    ...
    [dummy, nom(k,1), denom(k,1)] = theta_estimate(x_dense,mu,dt_small);
    % disp(dummy)
